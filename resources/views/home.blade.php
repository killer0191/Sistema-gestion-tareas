...
<body>
              ...
              <div class="d-flex justify-content-between">
                ...
                <div class="btn-group ml-auto">
                  <form method="POST" class="eliminar-tarea-form" action="/tarea/{{ $tarea->idTarea }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-accion eliminar-tarea-btn" data-action="eliminar"
                      data-id="{{ $tarea->idTarea }}">Eliminar</button>
                  </form>
                  <button class="btn btn-accion editar-tarea-btn" data-toggle="modal" data-target="#editarTareaModal"
  ...
  $(document).ready(function() {
    ...
    $("#crearTareaForm").submit(function(event) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          // Generar el HTML para la nueva tarea
          var imagen = response.idEstadoF == 1 ? 'finalizada.png' : response.fechaVenc ? 'pendiente.png' :
            'vencida.png';
          var nuevaTarea = `
        <div class="card mb-3">
            <img src="{{ asset('assets/') }}/${imagen}" class="estado-imagen" alt="Estado" width="50px">
    ...
    `;
          ...
          // Agregar la nueva tarea al contenedor de tareas existente
          $(".card-container").append(nuevaTarea);
        },
